...
function SimpleBiLSTM:createNetwork()
    ...
    local rnn = nn.Sequential()
        :add(brnn) 
        --:add(nn.Sequencer(nn.Sequential():add(nn.Dropout(0.5)):add(nn.MaskZero(nn.Linear(mergeHiddenSize, self.numLabels), 1)))) 
